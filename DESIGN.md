...
version: alpha
name: An · 雾蓝纸页 v2
description: ninesue.online 的设计系统(重构目标状态)——黛青主色、衬线长文排版、亮色纸张 / 暗色星空双世界观。2026-07 重构定稿。
colors:
  ...
  # 马克笔高亮(strong 元素)
  marker: "rgba(252, 220, 100, 0.5)"
  marker-dark: "rgba(255, 200, 80, 0.28)"
...
个人技术博客(Hugo,单仓无主题层),内容以操作系统 / 内核 / 云原生长文为主。设计目标:**长文阅读舒适度优先**,视觉克制、带一点手绘温度。
...
5. **马克笔 = 站点视觉签名**:亮色暖黄 `rgba(252,220,100,.5)`(白纸真笔触),暗色冷色荧光 `rgba(130,210,230,.32)`(深灰纸上暖黄是脏色,冷色才有对比)。固定出现在正文 `hr` 分割线和标签筛选器激活按钮;正文段落/引用/列表内 hand-written strong 由 marker.js 随机笔触渲染(仅作用 p/blockquote/li 内的 b/strong,不碰代码/标题)。TOC 激活项是干净的 border-left + 主色淡底,**不是**马克笔——导航不是强调。(首页站名试过笔触,不好看,已否决——不要再来。)

核心世界观——**亮暗两套性格**:

- **亮色 = 白纸**:纯白背景,零装饰(网格/SVG/梅花/雨滴均已被否决,不要任何粒子和动效)。
...
- 马克笔:亮色暖黄 `rgba(252,220,100,.5)`(白纸真笔触),暗色冷色荧光 `rgba(130,210,230,.32)`(深灰纸上才有对比)。是全站唯一的"强调"颜色语言
- **规则:不新增裸 hex。**一律 `var(--*)`;变量不够先在 `body` / `body.dark` 上补。

## Typography

- 正文:**Noto Serif SC 中文衬线**,18px / 1.75 行高;行宽 768px ≈ 每行 40 汉字。不换无衬线、不加宽、不缩字号。
- 标题:font-weight 500、line-height 1.3,字号 30/24/20/18/16px(桌面),移动端降一档(24/20/18/17/16px)。
- 代码:Fira Code,块代码 600 字重,行内 0.9em / 500 字重 + 主色淡背景。首页站名也用 Fira Code(DM Mono 已移除)。
...
- 内容主宽度 768px;文章纸张容器 900px 居中,**内文另限 40em(≈40 汉字/行)居中于纸内**——纸可以宽,字不行。
- 博客列表页:单列 768px 居中。标题下方是标签筛选条(Fira Code 12px 按钮,选中=马克笔划过),列表区按年份分隔线(年标 Fira Code 12px + 细线)分组。每篇文章显示黛青标题 + 行内标签(Fira Code 10px) + `Jan 2 · 3 min` 日期栏(固定 7.5em 宽右对齐)。旧侧栏已退役。
...
三层材质语言,新组件先归类,不发明第四种:

1. **平面**(列表页、友链页):无阴影,靠边框和背景色区分。
2. **纸张**(文章页):双层柔和阴影 + noise.png 纹理(亮 3% / 暗 6%);暗色纸张 `#1e1e1e` + 蓝色微光 `0 0 30px rgba(111,143,209,0.08)`。
3. **毛玻璃**(TOC 面板):`rgba(255,255,255,0.85)` + blur(12px),暗色 `rgba(9,12,16,0.85)`。

## Shapes

- 圆角刻度收敛为 4 档:**0(图片/详情块)、2px(行内代码)、8px(卡片/面板/代码块)、16px(抽屉/友链卡)、全圆(头像/圆钮)**。
- 旧的 6px 和 10px 在重构中一律迁移到 8px;不再新增其他圆角值。
- 马克笔效果(marker.js):每个 `strong` 随机笔触,亮暗同为暖黄。**加粗即高亮**,不另造高亮样式;静态签名两处(hr/TOC 激活)在 sass 中用固定角度笔触实现。

## Components

- **paper-card**:文章页容器。900px、圆角 8px、noise 纹理、双层阴影;移动端左右留 10px。
- **friend-card**:56px 圆头像 + 两行文本;hover 背景变主色淡色、头像叠遮罩 + `→`;入场淡入,顺序随机(Fisher-Yates)。
- **tag-filter-bar**:文章列表标题下方一排横排标签按钮,选中即暖黄/冷色荧光马克笔划过。每篇文章日期旁有极小的行内标签(Fira Code 10px,`--text-decoration-color`)。文章页底部亦有同款文末标签。旧侧栏五彩胶囊已退役。
- **toc-panel**:毛玻璃、H2–H6 逐级缩小、折叠 400ms、激活项 `border-left: 3px var(--primary-color)` + 主色淡底(不是马克笔——导航≠强调)。
- **mobile-toc-btn**:48px 主色圆钮,fixed 右下。
- **文章页信息**:标题下方 `日期  #标签 · 3 min · 1003 字`(阅读时长+字数跟在标签行末)。正文 `40em` 限宽敞纸。文末一行淡色 Fira Code 文章标签。
- **首页签名**:`明道若昧,进道若退`(道德经四十一章)——bio + meta description + og:description 三处一致。
...
- 不给正文第二种高亮;标题不加粗过 500;代码块字重保持 600。
- 不加宽 768px 行宽、不缩小 18px 正文。
- 不在列表页/友链页加纸张材质——平面是刻意的。
